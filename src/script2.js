...
const floor = new THREE.Mesh(
	new THREE.BoxGeometry(100, 100, 0.01),
	new THREE.MeshStandardMaterial({
	  color: "#ffffff",
	  metalness: 0.3,
	  roughness: 0.4,
...
let character = new GLTFLoader().load(Avatar, function (gltf) {
  ...
  // console.log("modelBody", modelBody.position);

  animations = new THREE.AnimationMixer(model);
  const action = animations.clipAction(gltf.animations[0]);
  action.play();
...
window.addEventListener("keydown", (e) => {
  switch (e.key) {
    case "w":
      modelBody.position.z += 0.1;
      break;
    case "s":
      modelBody.position.z -= 0.1;
      break;
    case "a":
      modelBody.position.x -= 0.1;
      break;
    case "d":
      modelBody.position.x += 0.1;
      break;
